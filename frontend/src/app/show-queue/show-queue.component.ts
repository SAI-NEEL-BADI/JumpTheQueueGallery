...
export class ShowQueueComponent implements OnInit {
 ...
 joinQueue(eventName: string, eventId: number){
   ...
   this.eventService.joinQueue(this.joinEvent).subscribe(data => {
    this.msg = data;
    this.eventService.getVisitorEventQueueDetails(this.visitorCriteria).subscribe( queueData => {
    ...
    if (queueItem.eventId === eventId){
      ...
      for (const item of this.localEvents){
        if (this.queueDetail.eventId === item.id){
          this.queueDetail.currentlyBeingAttended = item.currentlyBeingAttended;

          //this.queueDetail.estimatedTime = new Date();

          // tslint:disable-next-line: max-line-length
         // this.queueDetail.estimatedTime.setMinutes(this.queueDetail.estimatedTime.getMinutes() + ((item.attentionTime / 1000) * (item.visitorsCount-1)));
        }
        localStorage.setItem('currentQueue', JSON.stringify( this.queueDetail ));
      }
    }
  }
 }
